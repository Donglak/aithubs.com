...
const ToolDetailPage = () => {
  ...
  const toolBase = useMemo(
    () => tools.find((t) => toSlug(t.name) === (slug ?? "")),
    [slug],
  );
=======
...
<<<<<<< HEAD
  const tool = useMemo(() => buildToolDetails(toolBase as any), [toolBase]);
...
<<<<<<< HEAD
        ...
        <link
          rel="canonical"
          href={tool.seo?.canonical ?? `https://aithubs.com/tools/${tool.name}`}
        />
...
<<<<<<< HEAD
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-gray-900 dark:text-gray-300">
                    <span className="inline-flex items-center gap-1">
                      <Star className="w-4 h-4 text-yellow-500" />{" "}
                      {(tool.rating ?? 0).toFixed(1)} ({tool.reviews} reviews)
                    </span>
                    <span className="inline-flex items-center gap-1">
                      <Award className="w-4 h-4" /> {industries.join(" / ")}
                    </span>
                    {tool.company && (
                      <span className="inline-flex items-center gap-1">
                        <Users className="w-4 h-4" /> {tool.company}
                      </span>
                    )}
=======
                    ...
                    <div>
                      ...
                      <ul className="space-y-1">
                        {tool.useCases?.map((f, idx) => (
                          <li key={idx} className="flex items-start gap-2">
                            <Check className="w-4 h-4 mt-1 text-green-600" />
                            <span className="text-gray-700 dark:text-gray-300 whitespace-nowrap overflow-hidden text-ellipsis block">
                              {f}
                            </span>
                          </li>
                        ))}
                    ...
                    <div className="mt-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {tool.idealFor.map((seg, i) => (
                          <span
                            key={i}
                            className="px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-xs"
                          >
                            {seg}
                          </span>
                        ))}
                    ...
                    {relatedTools.map((relatedTool) => (
                      ...
                      >
                        ...
                        <div className="flex items-center">
                          ...
                          <span className="text-sm text-gray-900 dark:text-gray-300 ml-1">
                            {relatedTool.rating}
                          </span>
                        </div>
                      </Link>
