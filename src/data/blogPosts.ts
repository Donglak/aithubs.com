...
import nocodeRaw1 from "../content/posts/no-code-tools.md?raw";
...
const ultimateHTML = marked(
  applyHighlightSyntax(stripFrontmatter(ultimateRaw)),
);
const chatgptHTML = marked(applyHighlightSyntax(stripFrontmatter(chatgptRaw)));
const affiliateHTML = marked(
  applyHighlightSyntax(stripFrontmatter(affiliateRaw)),
);
const automationHTML = marked(
  applyHighlightSyntax(stripFrontmatter(automationRaw)),
);
const nocodeHTML = marked(applyHighlightSyntax(stripFrontmatter(nocodeRaw)));
const nocodeHTML1 = marked(applyHighlightSyntax(stripFrontmatter(nocodeRaw1)));
...
const nocodeExcerpt1 = extractFrontmatterExcerpt(nocodeRaw1);
...
export const BLOG_POSTS: BlogPost[] = [
  ...
  },
  {
    id: 7,
    slug: "no-code-tools",
    title: "claude with nocode",
    content: nocodeHTML1,
    excerpt: nocodeExcerpt1,
    category: "comparisons",
    author: "Alex huang",
    date: "2024-01-03",
    readTime: "11 min read",
